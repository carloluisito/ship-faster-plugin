...
- **Claims.** The PostToolUse edit hook records, per session, every file the session changes through Edit, Write, MultiEdit, or NotebookEdit (`<projectDir>/edits/<sid>.json`: path → time and tool). Changes made through Bash commands are not attributable and count as unclaimed.
...
- **Open session.** A session whose record still exists (SessionEnd deletes it). Claims of ended sessions are `earlier`.
...
- **Mode.** `shared` when another open session has a current claim, or another session record was active in the last two hours (its branch must not move). Otherwise `solo`. Without a session id, `solo`. `--here` forces `solo`.
...
- **Shared:**
  ...
  - After the commit, `worktree.mjs clear --from <path>` removes the shipped changes from the folder: a file identical to the committed one is restored to HEAD; a file that also holds another session's hunks has the committed patch reverse-applied; a file changed since carrying is kept and reported.
  ...
  - A both-sessions file can be carried with only this session's hunks (interactive choice); the reverse-apply in `clear` then leaves the other session's hunks in the folder.
...
- An idle session record older than two hours without claims does not trigger shared mode (closed terminals skip SessionEnd). A record with current claims does, at any age up to the seven-day prune. Cost if wrong: one extra question after a crash.
...
- `lib/state.mjs`: `editsFile(root, sid)`, `recordEdit(root, sid, rel, { tool, at })` (merge-safe, capped at 1000 paths), `loadEdits(root)`, `dropClaims(root, rels, { sid })`, `sessionRecords(root)`; `pruneSessions` also prunes `edits/`.
- `lib/git.mjs`: `git(args, { input })`; `dirtyFiles` entries gain `from` for renames and copies; `lastCommitTimes(cwd, paths)`; `blobIds(cwd, ref, paths)`; `hashFiles(cwd, paths)`.
- `lib/ownership.mjs`: `ownership(root, { sid, dirty, includes, here, now })` → `{ session, mode, forced, others, owners, ship, ask, leave }`.
- `changes.mjs --session <sid> [--include <globs>] [--here]`: adds `ownership` and per-entry `owner`.
