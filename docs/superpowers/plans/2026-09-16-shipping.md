...
- [ ] **Step 3: Add the git helpers, and unquote non-ASCII paths in log output**

Plan 2's final review found that `git log --name-only` quotes non-ASCII paths (`"src/caf\303\251.js"`) under the default `core.quotepath`, so such a path never equals a page's `rel` and the alongside rule silently fails for it. In `git()` in `lib/git.mjs`, prepend `-c core.quotepath=false` to every invocation: change `spawnSync('git', args, {` to `spawnSync('git', ['-c', 'core.quotepath=false', ...args], {`. Add this test to `plugins/ship-faster/tests/git.test.mjs`:

```js
test('log output carries non-ASCII paths unquoted', () => {
  const { root, git } = makeRepo({ files: { 'a.txt': 'a\n' } });
  const name = 'café.txt';
  writeFileSync(join(root, name), 'c\n');
  git(['add', name]);
  git(['commit', '-q', '-m', 'accent']);
  const entry = g.log(root, { n: 1 })[0];
  assert.deepEqual(entry.files, [name]);
  assert.deepEqual(g.commitsSince(root, `HEAD~1`).commits[0].files, [name]);
});
```

(The `é` escape lives inside a JavaScript string in the test file, where it is legitimate; it is not typed into a tool call as a raw escape.)

Then append to `plugins/ship-faster/scripts/lib/git.mjs`:
...
- [ ] **Step 4: Restore the template's ship clause**

Now that `ship` exists, put the clause back in `plugins/ship-faster/templates/claude-md.md` (plan 2's final review removed it as false at the time). Replace the "Keeping docs true" line
...
and make the same edit in this repository's own `CLAUDE.md` (the managed block, "Keeping docs true"). Run `node --test plugins/ship-faster/tests/templates.test.mjs` and `node plugins/ship-faster/scripts/lint.mjs`.
...
```bash
git add plugins/ship-faster/skills/ship plugins/ship-faster/evals/ship plugins/ship-faster/templates/claude-md.md CLAUDE.md
git commit -F <message file>
```
...
Also re-check the SessionStart budget against a long history, which plan 2's final review flagged: build a temp repository with 600 commits after the verified commit of 30 pages that share one sha (adapt `tests/bench.mjs`'s fixture in a scratch script under the SDD workspace, not committed) and time `hook-session-start.mjs`; it must stay under 1500 ms. If it does not, lower `LOG_LIMIT` in `scripts/stale.mjs` for the hook path or switch the one-sha path back to `git diff` when the commit count exceeds 300, with a test.
