...
covers: [plugins/ship-faster/scripts/**, plugins/ship-faster/tests/validate.mjs, .gitattributes]
...
- `ask` as a guard default — its behaviour under bypass-permissions mode is undocumented (`plugins/ship-faster/README.md:77`). Instead: default to `deny`.
...
| Skill frontmatter | known fields only; `name` equals the directory; `description` and `allowed-tools` present; `when_to_use` unless `disable-model-invocation`; description plus when_to_use at most 1536 characters; SKILL.md at most 500 lines | `plugins/ship-faster/tests/validate.mjs` |
