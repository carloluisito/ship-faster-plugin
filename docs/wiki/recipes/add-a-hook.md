...
2. Print only when there is something to say. SessionStart and UserPromptSubmit print plain text; a PreToolUse decision prints `hookSpecificOutput` JSON, as `plugins/ship-faster/scripts/hook-ship-guard.mjs:209` does; a `warn` prints `additionalContext` in it instead of a `permissionDecision`.
3. End with `main().catch(() => {}).finally(() => { process.exitCode = 0; });`. If tests must import functions from it, use the `process.argv[1]` entry guard instead, as `hook-ship-guard.mjs:212` does.
