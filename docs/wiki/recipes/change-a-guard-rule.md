...
5. For a new rule name, add it with value `deny` to `DEFAULTS.guard` in `plugins/ship-faster/scripts/lib/config.mjs`; `loadConfig` rejects `guard` keys that are not in the defaults.
