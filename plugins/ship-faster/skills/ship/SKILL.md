...
- `shared`: other sessions use this checkout (`ownership.others`: branch, last activity, files they changed). `ownership.ship` holds this session's files plus the `--include` matches.
  ...
  - `ownership.ask` lists files whose owner is `both` (this session and another open one changed it), `earlier` (only a session that has ended changed it), or `unclaimed` (no session's edits explain it, such as the output of a Bash command). Ask the user once, listing each file with its owner, which to add; a `both` file can be added whole or with only this session's hunks. In a non-interactive run add none and list them.
